...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Настройка турниров для конкретного вида спорта
        ...
        await query.edit_message_text(
            f"Настрой турниры {config.SPORTS.get(sport, sport)}:\n\n"
            f"• <b>Все турниры</b> - получать все прогнозы\n"
            f"• <b>Выбрать конкретные</b> - только по этим турнирам\n"
            f"• <b>Добавить вручную</b> - если нужного нет в списке",
            parse_mode='HTML',
    ...
    # Кнопка "Все турниры"
        ...
        # Очищаем все выбранные турниры для этого спорта = получать все
    ...
    # Добавить турнир вручную
        ...
        # Сохраняем спорт в контексте пользователя
        context.user_data['waiting_for_tournament'] = sport
        
        await query.edit_message_text(
            f"Напиши название турнира {config.SPORTS.get(sport, sport)}, который хочешь добавить.\n\n"
            f"Например: <code>Кубок Италии</code>\n\n"
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'waiting_for_tournament' in context.user_data:
        ...
        await update.message.reply_text(
            f"Отменено. Возвращаю к настройкам турниров:",
            reply_markup=keyboards.get_tournaments_keyboard(sport, user_tournaments)
        )
    else:
...
async def text_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'waiting_for_tournament' in context.user_data:
        ...
        await update.message.reply_text(
            f"✅ Турнир «{tournament_name}» добавлен!\n\n"
            f"Теперь ты будешь получать прогнозы по этому турниру.",
            reply_markup=keyboards.get_tournaments_keyboard(sport, user_tournaments)
        )
        return

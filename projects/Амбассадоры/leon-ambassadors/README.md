# Амбассадоры Leon

## Описание
Виджет с карточками амбассадоров букмекерской компании Leon в стиле Sports.ru

## Особенности
- Адаптивная сетка карточек амбассадоров
- Поддержка тёмной и светлой темы с автоматическим определением
- Современный дизайн с использованием брендовых цветов Sports
- Hover-эффекты на карточках
- Оптимизированная загрузка изображений
- Данные амбассадоров вынесены в `data/ambassadors.js`
- Есть fallback-данные в `index.html` для single-file деплоя на CDN

## Амбассадоры

### Виктор Логинов
**Роль:** Актер

## Файлы
- `index.html` — разметка виджета
- `data/ambassadors.js` — данные амбассадоров (подключается до скриптов с CDN)

## Деплой
- Рекомендуется загружать папку целиком, чтобы работал `./data/ambassadors.js`.
- Если загружен только `index.html`, карточка все равно отрисуется из fallback-данных.

### Скрипты и стили (CDN)
- Стили: [b76406bc50a074296f3b767b74187.css](https://dumpster.cdn.sports.ru/7/1a/b76406bc50a074296f3b767b74187.css)
- Отрисовка карточек: [12ca628dde403ee569e0c77f82a2a.js](https://dumpster.cdn.sports.ru/2/fd/12ca628dde403ee569e0c77f82a2a.js)
- Тема: [84affb661f8e40fc341dd126c5205.js](https://dumpster.cdn.sports.ru/1/c6/84affb661f8e40fc341dd126c5205.js)
- Resize iframe: [9366c209a59afd51a4e10c9d1215b.js](https://dumpster.cdn.sports.ru/2/c4/9366c209a59afd51a4e10c9d1215b.js)
